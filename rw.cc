...
int main(int argc, char** argv) {
  ...
  **********************************************************************************/
  ...
  try {
    ...
    std::cout << "Next RG start point: " << out_file->Tell().ValueOrDie() << "\n";
    int64_t start = out_file->Tell().ValueOrDie();
    ...
    for (int i = 0; i < NUM_ROWS; i++) {
      ...
      int64_t curr_bytes = rg_writer->total_bytes_written() + rg_writer->total_compressed_bytes();
      if ((curr_bytes + estimated_bytes) > ROW_GROUP_SIZE) {

        // int64_t padding_bytes = ROW_GROUP_SIZE - total;
        // std::cout << "Total bytes for the row group: " << total << "\n";
        // std::cout << "Padding: " << padding_bytes << "\n";
        // std::string padding_str = "";
        // for (int64_t k = 0; k < padding_bytes; k++) {
        //   padding_str += "k";
        // }
        // out_file->Write(std::make_shared<arrow::Buffer>(padding_str));
        rg_writer->Close();
        std::cout << "Next RG start point: " << out_file->Tell().ValueOrDie() << "\n";
        int64_t end = out_file->Tell().ValueOrDie();

        int64_t width = end - start;
        
        int64_t padding_bytes = ROW_GROUP_SIZE - width;
        std::string padding_str = "";
        for (int64_t k = 0; k < padding_bytes; k++) {
          padding_str += "x";
        }
        ...
        start = out_file->Tell().ValueOrDie();
        std::cout << "Next RG start point (padding): " << out_file->Tell().ValueOrDie() << "\n";
      ...
      bool_writer->WriteBatch(1, &def_level, nullptr, &bool_value);
      ...
      int32_writer->WriteBatch(1, &def_level, nullptr, &int32_value);
      ...
      int96_writer->WriteBatch(1, &def_level, nullptr, &int96_value);
      ...
      float_writer->WriteBatch(1, &def_level, nullptr, &float_value);
      ...
      double_writer->WriteBatch(1, &def_level, nullptr, &double_value);
      ...
      flba_writer->WriteBatch(1, &def_level, nullptr, &flba_value);
